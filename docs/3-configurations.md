...
A third file, the **model config** (`config.yaml`), comes with downloaded weights in `model_dir` and describes the trained network architecture. It is read automatically by the BMI and **should not be edited** -- changing this can desynchronize the code from the weights.
...
| `verbose` | bool | false | Enable output messages during runtime for debugging. |
| `pet_method` | str | `penman_monteith` | BMI-internal potential evapotranspiration calculation method. `penman_monteith` or `hargreaves`. |
| `latitude` | float (deg) | *required if Hargreaves* | Catchment centroid latitude. Converted to radians internally. |
| `start_time` | str | *required if Hargreaves* | Simulation start as `YYYY/MM/DD HH`, `YYYY/MM/DD HH:MM`, or `YYYY/MM/DD HH:MM:SS`. Used to derive day-of-year for Hargreaves and to stamp daily aggregates. Must match the realization's `time.start_time`. |

The BMIs (`dhbv2.bmi.DeltaModelBmi`) compute PET internally. The two supported PET methods have different **forcing** requirements, so this choice constrains the realization's `variables_names_map`:
...
* `hargreaves` — derives PET from the daily min/max/mean of temperature alone, plus `latitude` and day-of-year (`start_time`). Only precipitation and temperature are required.

### (2) Catchment Attributes

The remaining top-level keys are the 28 (daily) or 30 (MTS) static attributes the parameterization network expects. These must be sourced from the same attribute set the model was trained on; substituting a differently-derived values may degrade predictions.

Each attribute is exposed through BMI under a CSDMS standard name. The internal name is used in the yaml:
...
A missing attribute is **not fatal**: the BMI logs `Static variable '<name>' not in BMI config. Skipping.` and leaves the value at `0.0`, which will propagate into the parameterization. [This could change in future revisions.]

### (4) MTS-Only Keys: `warmup`

> Coming later to daily dhbv2
...
* `periodic` *(default, legacy behavior)* -- re-initialize from default states over the full `daily_warmup_days` at every warmup. Most expensive, most reproducible.
...
The choice directly sets **how much forcing data must precede your first prediction**.
...
So with defaults, a simulation starting `2009-01-01 00:00` needs a forcing timeseries beginning `2008-01-08 00:00`. Shortening the lead-in with `cold` modes trades spin-up cost for less-saturated initial states -- useful for smoke tests, not for benchmarking.
...
> (i) ngen assumes `precip_rate` is **mm s-1** unless the header carries an explicit unit, which is why the shipped CSVs use `precip_rate[mm h-1]`. The NetCDF realization maps the bare `precip_rate` instead, because the NetCDF variable carries its own units attribute.
>
...
`start_time` is the first *predicted* step, not the first step of *data*. Your forcing file must begin one full warmup lead-in earlier -- 358 days for MTS defaults, 365 days for the daily model.
